...
class C_report_invoice_ict_entry extends CI_Controller {
    ...
    function index(){
        ...
        $arraydata = $this->mbatps->query("SELECT DISTINCT Consignee,Consignee 'Consignee2' 
            from tps_t_manual_invoice_head where status not in ('D','V') order by Consignee")->result_array();
    ...
    function c_export(){
        ...
        $query.= "  FROM tps_t_manual_invoice_head a " ;
        $query.= "  INNER JOIN ptmsa_dbo.whs_vw_customer b  ON a.cust_code=b.cust_code " ;
        ...
        //Setting Sheet Excel
        $nama_excel = "Report_InvEntry_Ict_".tanggal_sekarang() ;

        $nama_sheet = array(
            '0' => 'List Invoice Entry ICT',
        );
