...
class C_sppb_bc extends CI_Controller {
    ...
    function index(){
        ...
        $arraydata = array(
            'GetSppb_Bc23' => 'Per SPPB',
            'GetBC23Permit' => 'Per Gudang',
            'GetBC23PErmit_FASP' => 'Per Kode TPS'
        );
        $nmservice = ComboNonDb($arraydata, 'nmservice', 'GetBC23Permit', 'form-control form-control-sm');
    ...
    function c_load_data(){
        ...
        $this->db_tpsonline->select(" ID,CAR,NO_sppb,DATE_FORMAT(TGL_sppb,'%d-%m-%Y') 'TGL_sppb',KD_KANTOR_PENGAWAS,KD_KANTOR_BONGKAR,
            NO_PIB,DATE_FORMAT(TGL_PIB,'%d-%m-%Y') 'TGL_PIB',
            NPWP_IMP,NAMA_IMP,ALAMAT_IMP,
            NPWP_PPJK,NAMA_PPJK,ALAMAT_PPJK,NM_ANGKUT,NO_VOY_FLIGHT,BRUTTO,NETTO,
            GUDANG,STATUS_JALUR,JML_CONT,NO_BC11,DATE_FORMAT(TGL_BC11,'%d-%m-%Y'),NO_POS_BC11,NO_BL_AWB,
            ifnull(DATE_FORMAT(TGL_BL_AWB,'%d-%m-%Y'),'') 'TGL_BL_AWB',NO_MASTER_BL_AWB,
            ifnull(DATE_FORMAT(TGL_MASTER_BL_AWB,'%d-%m-%Y'),'') 'TGL_MASTER_BL_AWB' ") ;
        $this->db_tpsonline->order_by('ID desc');
        $data_sppb23 = $this->db_tpsonline->get('tbl_respon_sppb_23');

        $data_sppb23_cont = $this->db_tpsonline->get_where('tbl_respon_sppb_23_container',array('ID' => $ID));

        $data_sppb23_kms = $this->db_tpsonline->get_where('tbl_respon_sppb_23_kms',array('ID' => $ID));

        $data = array(
            'data_sppb23' => $this->m_model->array_tag_on_index($data_sppb23->result_array()),
            'data_sppb23_cont' => $this->m_model->array_tag_on_index($data_sppb23_cont->result_array()),
            'data_sppb23_kms' => $this->m_model->array_tag_on_index($data_sppb23_kms->result_array()),
            'jml_data_sppb23' => $data_sppb23->num_rows(),
            'jml_data_sppb23_cont' => $data_sppb23_cont->num_rows(),
            'jml_data_sppb23_kms' => $data_sppb23_kms->num_rows(),
        );
    ...
    function c_download(){
        ...
        $No_Sppb23 = $this->input->post('No_Sppb23') ;
        $Tgl_Sppb23 = $this->input->post('Tgl_Sppb23') ;
        ...
        if($nmservice == "GetSppb_Bc23"){

            $xml_param = replace_xml_condition('No_Sppb','string',$No_Sppb23,$xml_param);
            $xml_param = replace_xml_condition('Tgl_Sppb','string',$Tgl_Sppb23,$xml_param);
            $xml_param = replace_xml_condition('NPWP_Imp','string',$NPWP_Imp,$xml_param);


        }else if($nmservice == "GetBC23Permit"){
        ...
        $ID = $this->m_model->select_max_where('db_tpsonline','tbl_respon_sppb_23', 'ID');
        ...
        if(Search_Str_To_Str($str_respon,'<DOCUMENT>') == 1){
            ...
            if($nmservice == "GetSppb_Bc23"){
                preg_match_all("#<GetSppb_Bc23Result>(.+?)</GetSppb_Bc23Result>#", $str_respon, $matchesx);
            }else if($nmservice == "GetBC23Permit"){
                preg_match_all("#<GetBC23PermitResult>(.+?)</GetBC23PermitResult>#", $str_respon, $matchesx);
            }else{
                preg_match_all("#<GetBC23Permit_FASPResult>(.+?)</GetBC23Permit_FASPResult>#", $str_respon, $matchesx);
            }
        ...
        for($n = 0 ; $n < count($data_validasi) ; $n++ ){
            ...
            for($col = 0 ; $col < count($data_sppb) ; $col++){
                ...
                if($col == 2 || $col == 6 || $col == 21 || $col == 24 || $col == 26 ){
        ...
        if(count($save_sppb) > 0){
            $pesan = "" ;
            $hasil = $this->db_tpsonline->insert_batch('tbl_respon_sppb_23', $save_sppb);
            $query[$a++] = $this->db_tpsonline->last_query();
            if ($hasil >= 1) {
                ...
                $pesan_data = array(
                    'msg' => 'Tidak',
                    'pesan' => 'Function Save tbl_respon_sppb_23 Error....!!!!',
                    'query' => $query,
                );
                echo json_encode($pesan_data);die;
            }
        ...
        if(count($save_sppb_cont) > 0){
            $pesan = "" ;
            $hasil = $this->db_tpsonline->insert_batch('tbl_respon_sppb_23_container', $save_sppb_cont);
            $query[$a++] = $this->db_tpsonline->last_query();
            if ($hasil >= 1) {
                $pesan = "Simpan Data Berhasil..." ;
            } else {
                $pesan_data = array(
                    'msg' => 'Tidak',
                    'pesan' => 'Function Save tbl_respon_sppb_23_container Error....!!!!',
                    'query' => $query,
                );
                echo json_encode($pesan_data);die;
            }
        ...
        if(count($save_sppb_kms) > 0){
            $pesan = "" ;
            $hasil = $this->db_tpsonline->insert_batch('tbl_respon_sppb_23_kms', $save_sppb_kms);
            $query[$a++] = $this->db_tpsonline->last_query();
            if ($hasil >= 1) {
                $pesan = "Simpan Data Berhasil..." ;
            } else {
                $pesan_data = array(
                    'msg' => 'Tidak',
                    'pesan' => 'Function Save tbl_respon_sppb_23_kms Error....!!!!',
                    'query' => $query,
                );
                echo json_encode($pesan_data);die;
            }
