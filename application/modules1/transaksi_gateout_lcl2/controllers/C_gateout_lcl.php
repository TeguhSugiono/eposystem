...
class C_gateout_lcl extends CI_Controller {
    ...
    function c_kirimdata(){
        ...
        for($a=0 ;$a<count($arrREF_NUMBER);$a++){
            ...
            //BUAT HEADER
            $this->db_tpsonline->select('KD_DOK,KD_TPS,NM_ANGKUT,NO_VOY_FLIGHT,CALL_SIGN,TGL_TIBA,KD_GUDANG,REF_NUMBER');
            $this->db_tpsonline->where(array('REF_NUMBER' => $STR_REF_NUMBER,'NO_BL_AWB' => $STR_NO_BL_AWB,'SENDING <>' => 'D'));
            $sql = $this->db_tpsonline->get('tbl_request_plp_out_kemasan');
            $sql = $this->db_tpsonline->last_query();
            $query_data = $this->db_tpsonline->query($sql)->result_array();
            $query_tagname = $this->db_tpsonline->query($sql)->list_fields();
    ...
    function c_sinkron_to_gateoutlcl(){
        ...
        for($a=0;$a<count((array) $array_response);$a++){
            ...
            if($cek_reff_number == 1 &&  $cek_proses == 1){
                $hasil = $this->db_tpsonline->update('tbl_request_plp_out_kemasan',array('SENDING' => 'Y'),array('SENDING <>' => 'D','REF_NUMBER' => explode(' ',$str_replace_data)[1]));
    ...
    function c_change_reff_number(){
        ...
        $hasil = $this->db_tpsonline->update('tbl_request_plp_out_kemasan',
                        ...
                        array('REF_NUMBER' => $REF_NUMBER)) ; //,'NO_BL_AWB' => '360781'
    ...
    function c_sinkron_data(){
        ...
        foreach($getdata->result_array() as $adata){
            ...
            $query_in_data = " SELECT * from whs_t_out a INNER JOIN whs_t_out_detail b on a.batch_out=b.batch_out 
            ...
            if($excute_data->num_rows() > 0 ){
                ...
                foreach($excute_data->result_array() as $out_whs){
                    ...
                    $excute_data_lcl_in = $this->db_tpsonline->get_where('tbl_request_plp_in_kemasan',array('REF_NUMBER' => $REF_NUMBER_LCL_IN,'NO_BL_AWB' => $NO_BL_AWB));
                    ...
                    $excute_update = $this->m_model->updatedata('db_tpsonline','tbl_request_plp_out_kemasan', $array_update, 
                        array('REF_NUMBER' => $adata['REF_NUMBER'],'NO_BL_AWB' => $adata['NO_BL_AWB']));
    ...
    function c_change_reff_number_temp(){
        ...
        foreach($getdata->result_array() as $adata){
            ...
            $excute_update = $this->m_model->updatedata('db_tpsonline','tbl_request_plp_out_kemasan',
             array('REF_NUMBER' => $REF_NUMBER), array('REF_NUMBER' => $adata['REF_NUMBER'],'NO_BL_AWB' => $adata['NO_BL_AWB']));
    ...
    function c_delete_reff_number(){
        ...
        // $pesan_data = array(
        //     'msg' => 'Tidak',
        //     'array_REF_NUMBER' => explode('###',$array_REF_NUMBER[0])
        // );
        // echo json_encode($pesan_data);die;
        ...
        for($a=0 ;$a<count($arrREF_NUMBER);$a++){
            ...
            $hasil = $this->db_tpsonline->update('tbl_request_plp_out_kemasan',
                array('SENDING' => 'D'), 
                array('REF_NUMBER' => $STR_REF_NUMBER,'NO_BL_AWB' => $STR_NO_BL_AWB)) ;
