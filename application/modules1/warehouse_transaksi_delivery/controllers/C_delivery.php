...
class C_delivery extends CI_Controller {
    ...
    function c_save(){
        ...
        $query = " SELECT * from whs_t_out_check a 
            ...
            where a.rec_id<>9 and b.rec_id<>9 and b.bl_no = '".$do_no."' " ;
        ...
        $datasegel = $this->tribltps->get_where('whs_t_stock_detail',array('bl_no' => $bl_no,'flag_segel' => 'Y','flag_unsegel' => 'N'));
        $datasegelweb = $this->tribltps->get_where('tps_stop_sistem_lcl',array('bl_no' => $bl_no,'status_segel' => 'Y'));

        if($datasegel->num_rows() > 0){
            $comp = array(
                'msg' => 'Tidak',
                'pesan' => 'Segel Merah ..!!'
            );
