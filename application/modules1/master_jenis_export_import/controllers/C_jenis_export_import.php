...
class C_jenis_export_import extends CI_Controller {
    ...
    function c_tbl_jenis_export_import() {
        $database = "db_tpsonline";

        $select = 'id,jenis_export_import,keterangan';
        $form = 'tbl_m_tps_export_import';
        $join = array();
        $where = array('rec_id' => 0);

        $where_term = array(
            'id', 'jenis_export_import','keterangan'
        );
        $column_order = array(
            null, 'id', 'jenis_export_import','keterangan'
        );
        $order = array(
            'id' => 'desc'
        );
    ...
    function c_save(){
        ...
        $data = array(
            'jenis_export_import' => $jenis_export_import,
            'keterangan' => $keterangan, 
            'rec_id' => 0,
            'created_by' => $this->session->userdata('autogate_username'),
            'created_on' => tanggal_sekarang(),
        );
        $hasil = $this->m_model->savedata($database, 'tbl_m_tps_export_import', $data);
    ...
    function c_formedit(){
        ...
        $array_data = $this->m_model->table_tostring($database, '', 'tbl_m_tps_export_import', '', array('id' => $id), '');
        $data = array(
    ...
    function c_update(){
        ...
        $hasil = $this->m_model->updatedata($database, 'tbl_m_tps_export_import', $data, $where);
