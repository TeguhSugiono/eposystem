...
class C_gateout_lcl extends CI_Controller {
    ...
    function c_tbl_gateout_lcl(){
        ...
        // $tambah_where = array('a.REF_NUMBER' => 'MBA0230505507673');
        ...
        foreach ($list as $field) {
            ...
            //$gabung_ref_bl = $field->REF_NUMBER."###".$field->NO_BL_AWB ;

            $gabung_ref_bl = $field->REF_NUMBER."###".$field->NM_ANGKUT."###".$field->NO_VOY_FLIGHT."###".$field->CALL_SIGN."###".$field->NO_BL_AWB."###".$field->CONT_ASAL;

            if($field->SENDING == "N"){
                $row[] = "<div align='center'><input type='checkbox' id='idreffnumber$no' class='chkreffnumber' 
                        onclick='HandleClick(this.id,".'"'.$gabung_ref_bl.'"'.");' value='".$gabung_ref_bl."'></div>";   
            }else{
    ...
    function c_formedit(){
        ...
        $REF_NUMBER_SELECTED = $this->input->post('REF_NUMBER_SELECTED');
        ...
        for($a=0; $a<count($REF_NUMBER_SELECTED); $a++){
            $REF_NUMBER = $REF_NUMBER_SELECTED[2] ;
            $NM_ANGKUT = $REF_NUMBER_SELECTED[6] ;
            $NO_VOY_FLIGHT = $REF_NUMBER_SELECTED[7] ;
            $CALL_SIGN = $REF_NUMBER_SELECTED[8] ;
            $NO_BL_AWB = $REF_NUMBER_SELECTED[11] ;
            $CONT_ASAL = $REF_NUMBER_SELECTED[21] ;
        }

        $data['NM_ANGKUT'] = $NM_ANGKUT ;
        $data['NO_VOY_FLIGHT'] = $NO_VOY_FLIGHT ;
        $data['CALL_SIGN'] = $CALL_SIGN ;


        $array_search = $this->m_model->table_tostring('db_tpsonline', '', 'tbl_request_plp_out_kemasan', '', 
            array('REF_NUMBER' => $REF_NUMBER,'NM_ANGKUT' => $NM_ANGKUT,'NO_VOY_FLIGHT' => $NO_VOY_FLIGHT,'CALL_SIGN' => $CALL_SIGN,
                'NO_BL_AWB' => $NO_BL_AWB,'CONT_ASAL' => $CONT_ASAL), 
            '');
    ...
    function c_update(){
        ...
        $NO_BL_AWB = $this->input->post('NO_BL_AWB');
        $NO_BL_AWB_NEW = $this->input->post('NO_BL_AWB_NEW');
        $NO_MASTER_BL_AWB = $this->input->post('NO_MASTER_BL_AWB');
        ...
        $CONT_ASAL = $this->input->post('CONT_ASAL') ;
        $NM_ANGKUT = $this->input->post('NM_ANGKUT') ;
        $NO_VOY_FLIGHT = $this->input->post('NO_VOY_FLIGHT') ;
    ...
    function c_formkirimdata(){
        // $array_REF_NUMBER = $this->input->post('arrREF_NUMBER');

        // $arrREF_NUMBER = array();
        // $arrBL_NUMBER = array();

        // for($a=0; $a<count($array_REF_NUMBER); $a++){
        //     $comp = explode('###',$array_REF_NUMBER[$a]);

        //     array_push($arrREF_NUMBER, $comp[0]) ;
        //     array_push($arrBL_NUMBER, $comp[1]) ;
        ...
        $arrREF_NUMBER = array();
        $arrNM_ANGKUT = array();
        $arrNO_VOY_FLIGHT = array();
        $arrCALL_SIGN = array();
        $arrBL_NUMBER = array();
        $arrCONT_ASAL = array();

        for($a=0; $a<count($array_REF_NUMBER); $a++){
            array_push($arrREF_NUMBER, $array_REF_NUMBER[$a][0]) ;
            array_push($arrNM_ANGKUT, $array_REF_NUMBER[$a][1]) ;
            array_push($arrNO_VOY_FLIGHT, $array_REF_NUMBER[$a][2]) ;
            array_push($arrCALL_SIGN, $array_REF_NUMBER[$a][3]) ;
            array_push($arrBL_NUMBER, $array_REF_NUMBER[$a][4]) ;
            array_push($arrCONT_ASAL, $array_REF_NUMBER[$a][5]) ;
        }
        ...
        $data = array(
            'arrREF_NUMBER' => ArrayToString($arrREF_NUMBER),            
            'arrNM_ANGKUT' => ArrayToString($arrNM_ANGKUT),
            'arrNO_VOY_FLIGHT' => ArrayToString($arrNO_VOY_FLIGHT),
            'arrCALL_SIGN' => ArrayToString($arrCALL_SIGN),
            'arrBL_NUMBER' => ArrayToString($arrBL_NUMBER),
            'arrCONT_ASAL' => ArrayToString($arrCONT_ASAL),
            'proses' => $proses
        );
    ...
    function c_kirimdata(){
        ...
        $arrNM_ANGKUT = explode(',',$this->input->post('arrNM_ANGKUT'));
        $arrNO_VOY_FLIGHT = explode(',',$this->input->post('arrNO_VOY_FLIGHT'));
        $arrCALL_SIGN = explode(',',$this->input->post('arrCALL_SIGN'));
        $arrBL_NUMBER = explode(',',$this->input->post('arrBL_NUMBER'));
        $arrCONT_ASAL = explode(',',$this->input->post('arrCONT_ASAL'));
        ...
        for($a=0 ;$a<count($arrREF_NUMBER);$a++){
            $STR_REF_NUMBER = $arrREF_NUMBER[$a];
            $STR_NM_ANGKUT = $arrNM_ANGKUT[$a];
            $STR_NO_VOY_FLIGHT = $arrNO_VOY_FLIGHT[$a];
            $STR_CALL_SIGN = $arrCALL_SIGN[$a];
            $STR_BL_NUMBER = $arrBL_NUMBER[$a];
            $STR_CONT_ASAL = $arrCONT_ASAL[$a];

            //BUAT HEADER
            $this->db_tpsonline->select('KD_DOK,KD_TPS,NM_ANGKUT,NO_VOY_FLIGHT,CALL_SIGN,TGL_TIBA,KD_GUDANG,REF_NUMBER');
            $this->db_tpsonline->where(
                array('REF_NUMBER' => $STR_REF_NUMBER,
                    'NO_BL_AWB' => $STR_BL_NUMBER,
                    'NM_ANGKUT' => $STR_NM_ANGKUT,
                    'NO_VOY_FLIGHT' => $STR_NO_VOY_FLIGHT,
                    'CALL_SIGN' => $STR_CALL_SIGN,
                    'CONT_ASAL' => $STR_CONT_ASAL,
                    'SENDING <>' => 'D')
            );
            $sql = $this->db_tpsonline->get('tbl_request_plp_out_kemasan');
            $sql = $this->db_tpsonline->last_query();
    ...
    function c_sinkron_to_gateoutlcl(){
        ...
        for($a=0;$a<count((array) $array_response);$a++){
            ...
            if($cek_reff_number == 1 &&  $cek_proses == 1){
                $hasil = $this->db_tpsonline->update('tbl_request_plp_out_kemasan',array('SENDING' => 'Y'),
                    array('SENDING <>' => 'D',
                        'REF_NUMBER' => explode(' ',$str_replace_data)[1],
                        'CONSIGNEE <>' => ''
                    )
                );
    ...
    function c_change_reff_number(){
        ...
        // $pesan_data = array(
        //     'msg' => 'Tidak',
        //     'array_REF_NUMBER' => $array_REF_NUMBER,
        //     'jml' => count($array_REF_NUMBER),
        // );
        // echo json_encode($pesan_data);die;


        for($a=0 ;$a<count($array_REF_NUMBER);$a++){
            $STR_REF_NUMBER = $array_REF_NUMBER[$a][0];
            $STR_NM_ANGKUT = $array_REF_NUMBER[$a][1];
            $STR_NO_VOY_FLIGHT = $array_REF_NUMBER[$a][2];
            $STR_CALL_SIGN = $array_REF_NUMBER[$a][3];
            $STR_BL_NUMBER = $array_REF_NUMBER[$a][4];
            $STR_CONT_ASAL = $array_REF_NUMBER[$a][5];

            // $pesan_data = array(
            //     'msg' => 'Tidak',
            //     'array_REF_NUMBER' => $array_REF_NUMBER,
            //     'jml' => count($array_REF_NUMBER),
            //     'STR_REF_NUMBER' => $STR_REF_NUMBER,
            //     'STR_NM_ANGKUT' => $STR_NM_ANGKUT,
            //     'STR_NO_VOY_FLIGHT' => $STR_NO_VOY_FLIGHT,
            //     'STR_CALL_SIGN' => $STR_CALL_SIGN,
            //     'STR_BL_NUMBER' => $STR_BL_NUMBER,
            //     'STR_CONT_ASAL' => $STR_CONT_ASAL,
            // );
            // echo json_encode($pesan_data);die;

            $dataku = $this->db_tpsonline->get_where('tbl_request_plp_out_kemasan',
                array(
                    'REF_NUMBER' => $STR_REF_NUMBER,
                    'NM_ANGKUT' => $STR_NM_ANGKUT,
                    'NO_VOY_FLIGHT' => $STR_NO_VOY_FLIGHT,
                    'CALL_SIGN' => $STR_CALL_SIGN,
                    'NO_BL_AWB' => $STR_BL_NUMBER,
            ...
            $hasil = $this->db_tpsonline->update('tbl_request_plp_out_kemasan',
                            array('REF_NUMBER' => $REF_NUMBER_NEW,'REF_NUMBER_OLD' => $STR_REF_NUMBER,'VALIDATE' => 'N' ,'SENDING' => 'N'), 
                            array(
                                'REF_NUMBER' => $STR_REF_NUMBER,
                                'NM_ANGKUT' => $STR_NM_ANGKUT,
                                'NO_VOY_FLIGHT' => $STR_NO_VOY_FLIGHT,
    ...
    function c_sinkron(){
        ...
        $this->db_tpsonline->where_in('NO_BL_AWB',$arrBL_NUMBER);
    ...
    function c_sinkron_data(){
        ...
        $this->db_tpsonline->where_in('REF_NUMBER',$arrREF_NUMBER);
        $this->db_tpsonline->where_in('NM_ANGKUT',$arrNM_ANGKUT);
        $this->db_tpsonline->where_in('NO_VOY_FLIGHT',$arrNO_VOY_FLIGHT);
        $this->db_tpsonline->where_in('CALL_SIGN',$arrCALL_SIGN);
        $this->db_tpsonline->where_in('NO_BL_AWB',$arrBL_NUMBER);
        $this->db_tpsonline->where_in('CONT_ASAL',$arrCONT_ASAL);
        ...
        //     'arrBL_NUMBER' => $arrBL_NUMBER,
        //     'arrCONT_ASAL' => $arrCONT_ASAL,
        //     'query_search' => $this->db_tpsonline->last_query(),
        // );
        // echo json_encode($pesan_data);
        // die;
        ...
        foreach($getdata->result_array() as $adata){
            ...
            $cek_data_in = " SELECT * from whs_t_in a INNER JOIN whs_t_in_detail b on a.batch_in=b.batch_in 
            where a.rec_id=0 and b.rec_id=0 and b.bl_no='".$NO_BL_AWB."' and a.cont_no='".$CONT_ASAL."' limit 1 " ;

            $excute_data_in = $this->tribltps->query($cek_data_in);

            if($excute_data_in->num_rows() == 0 ){
                $pesan_data = array(
                    'msg' => 'Tidak',
                    'pesan' => 'Data Transaksi In LCL OPS Tidak Ditemukan ...!!',
            ...
            foreach($excute_data_in->result_array() as $in_whs){
                $batch_in = $in_whs['batch_in'] ;

                $query_out_data = " SELECT * from whs_t_out a INNER JOIN whs_t_out_detail b on a.batch_out=b.batch_out 
                where a.rec_id=0 and b.rec_id=0 and b.bl_no='".$NO_BL_AWB."' and b.batch_in = '".$batch_in."' ORDER BY b.t_stock_id desc limit 1  " ;
                     

                $excute_data = $this->tribltps->query($query_out_data);     
                ...
                if($excute_data->num_rows() > 0 ){
                    ...
                    foreach($excute_data->result_array() as $out_whs){
                        ...
                        $excute_data_lcl_in = $this->db_tpsonline->get_where('tbl_request_plp_in_kemasan',array('REF_NUMBER' => $REF_NUMBER_LCL_IN,'NO_BL_AWB' => $NO_BL_AWB,'CONT_ASAL' => $CONT_ASAL));
                        ...
                        $excute_update = $this->m_model->updatedata('db_tpsonline','tbl_request_plp_out_kemasan', $array_update, 
                            array('REF_NUMBER' => $adata['REF_NUMBER'],'NO_BL_AWB' => $adata['NO_BL_AWB'],'CONT_ASAL' => $adata['CONT_ASAL']));
    ...
    function c_unsinkron_data(){
        ...
        foreach($getdata->result_array() as $adata){
            $excute_update = $this->m_model->updatedata('db_tpsonline','tbl_request_plp_out_kemasan', array('FLAG_WHS' => 'N'), 
                array('REF_NUMBER' => $adata['REF_NUMBER'],'NO_BL_AWB' => $adata['NO_BL_AWB'],'NM_ANGKUT' => $adata['NM_ANGKUT'],
                    'NO_VOY_FLIGHT' => $adata['NO_VOY_FLIGHT'],'CALL_SIGN' => $adata['CALL_SIGN'],'CONT_ASAL' => $adata['CONT_ASAL']));
    ...
    function c_change_reff_number_temp(){
        ...
        foreach($getdata->result_array() as $adata){
            ...
            $excute_update = $this->m_model->updatedata('db_tpsonline','tbl_request_plp_out_kemasan',
             array('REF_NUMBER' => $REF_NUMBER), array('REF_NUMBER' => $adata['REF_NUMBER'],
                'NO_BL_AWB' => $adata['NO_BL_AWB'],'NM_ANGKUT' => $adata['NM_ANGKUT'],'NO_VOY_FLIGHT' => $adata['NO_VOY_FLIGHT'],
                'CALL_SIGN' => $adata['CALL_SIGN'],'CONT_ASAL' => $adata['CONT_ASAL']));
    ...
    function c_exportxls() {
        ...
        $nama_excel = "GateOutLCL_".$nm."_".tanggal_sekarang() ;
        ...
        $query.= " FROM tbl_request_plp_out_kemasan a " ;
    ...
    // function c_import_gate_out_lcl(){
    //     $this->load->view('import_gate_out_lcl');
    // }
