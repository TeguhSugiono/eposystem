...
class C_report_invoice_ict_out extends CI_Controller {
    ...
    function index(){
        ...
        $arraydata = $this->mbatps->query("SELECT DISTINCT Consignee,Consignee 'Consignee2' from tps_t_inv_header where rec_id=0 order by Consignee")->result_array();
    ...
    function c_proses(){
        ...
        $nama_table_1 = "inv_ict_out_".$username ;
        $query = "";
        ...
        $query.= " IFNULL(a.BiayaTerminal,0) AS Biaya_TPS,  (SELECT SUM(Jumlah)  " ;
        $query.= " from tps_t_inv_biaya b  where b.Inv_Number=a.Inv_Number and Rec_ID=0) AS Biaya_TPP, " ;
        ...
        $query.= " from tps_t_inv_biaya b  where b.Inv_Number=a.Inv_Number and Rec_ID=0) + IFNULL(a.PPN,0)  " ;
        $query.= " + IFNULL(a.BiayaAdm,0) +  IFNULL(a.BiayaLainTerminal,0)+IFNULL(a.BiayaLainMBA,0)) AS Jumlah, a.KdOut,a.biayalainmba " ;
        $query.= " FROM tps_t_inv_header AS a    " ;
    ...
    function c_export(){
        ...
        $nama_table_1 = "test.inv_ict_out_".$username ;

        $data = $this->mbatps->query("SELECT 'nomor' as 'No', inv_number 'Nomor Invoice',date_format(tgl_inv,'%d-%m-%Y') 'Tgl Invoice',consignee 'Consignee',(Biaya_TPS+Biaya_TPP) 'Subtotal',
        ...
        //Setting Sheet Excel
        $nama_excel = "Report_InvOut_Ict_".tanggal_sekarang() ;

        $nama_sheet = array(
            '0' => 'List Invoice Out (Bahandle) ICT',
        );
