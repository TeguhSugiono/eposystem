...
class C_report_invoice_pjt_entry extends CI_Controller {
    ...
    function index(){
        ...
        $arraydata = $this->mbatps->query("SELECT DISTINCT Consignee,Consignee 'Consignee2' 
            from pjt_t_manual_invoice_head where status not in ('D','V') order by Consignee")->result_array();
    ...
    function c_export(){
        ...
        $query.= "  FROM pjt_t_manual_invoice_head a " ;
        ...
        //Setting Sheet Excel
        $nama_excel = "Report_InvEntry_Pjt_".tanggal_sekarang() ;

        $nama_sheet = array(
            '0' => 'List Invoice Entry PJT',
        );
